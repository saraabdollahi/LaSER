...
def extraction(input_address, link_graph_address):
    
    input_dataset=pd.read_csv(input_address, sep="\t") ### columns: source, target
    #input_dataset=pd.read_pickle(input_address)
    link_graph=pd.read_csv(link_graph_address,sep="\t") ### columns: entity, links
    #link_graph=pd.read_pickle(link_graph_address)
    ...
    #merged_links=merged_links[["source","target","shared_incoming_cnt","shared_outgoing_cnt","source_incoming_cnt","target_incoming_cnt","source_outgoing_cnt", "target_ougoing_cnt"]]
    print(merged_links.columns)
    return(merged_links)
