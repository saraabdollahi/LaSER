package de.l3s.cleopatra.laser.coordinates;
...
import de.l3s.cleopatra.laser.util.EntityIdGeneratorFromFiles;
import de.l3s.cleopatra.laser.util.FileLoader;
import de.l3s.cleopatra.laser.util.FileName;
import de.l3s.cleopatra.laser.util.MapUtil;
...
public class CoordinatesPerEntityExtractor {
	...
	public static void main(String[] args) {
		...
		Config.init(Map.of("language", "en", "data_folder", "data/"));
		...
		try {
			...
			cpee.writeBaseCoordinates(inputFileCoordinates, writerBase, false);
			System.out.println("writePlaceCoordinates");
			cpee.writePlaceCoordinates(inputFilePlaces, writerPlace, false);
			System.out.println("writeBaseRelationsCoordinates");
			cpee.writeBaseRelationsCoordinates(writerBaseRelation, statsWriter, false);
			System.out.println("writeOtherRelationsCoordinates");
			cpee.writeOtherRelationsCoordinates(writerOtherRelation, statsWriter, false);
			System.out.println("writeAllCoordnates");
			cpee.writeAllCoordinates(writerAllCoordinates, statsWriter, true);
	...
	private void writeAllCoordinates(String allCoordinatesFileName, PrintWriter statsWriter, boolean writeFile) {
	...
	public void writeBaseCoordinates(String inputFileCoordinates, String outputFileName, boolean writeFile) {
				...
				while (it.hasNext()) {
					...
					if (currentEntity != null && !entity.equals(currentEntity)) {
						if (longitude != null && latitude != null) {
							...
							writeLineWithOneProperty(writer, currentEntity, coordinate, null, BASE_PROPERTY, writeFile);
	...
	public void writePlaceCoordinates(String inputFilePlaces, String outputFileName, boolean writeFile) {
		Set<String> writtenEntities = new HashSet<String>();
		PrintWriter writer = null;
		try {
			...
			try {
				...
				while (it.hasNext()) {
					...
					if (coordinate != null && !this.entitiesWithCoordinates.containsKey(entity)) {
						...
						writeLineWithOneProperty(writer, entity, coordinate, place, PLACE_PROPERTY, writeFile);
	...
	public void writeBaseRelationsCoordinates(String outputFileName, PrintWriter statsWriter, boolean writeFile) {
		Set<String> writtenEntities = new HashSet<String>();
				...
				try {
					...
					while (it.hasNext()) {
						...
						if (!this.entitiesWithCoordinates.containsKey(subject)) {
							writeLineWithOneProperty(writer, subject, coordinate, object, property, writeFile);
	...
	public void writeOtherRelationsCoordinates(String outputFileName, PrintWriter statsWriter, boolean writeToFile) {
		Set<String> writtenEntities = new HashSet<String>();
						...
						if (currentRelation == null) {
							...
							if (this.places.contains(currentRelation.getObject())) {
								...
								if (propertiesValid) {
									...
									processRelation(writer, currentRelation, writeToFile);
	...
	private void processRelation(PrintWriter writer, Relation relation, boolean writeToFile) {
	...
	private void writeLineWithOneProperty(PrintWriter writer, String entity, Coordinate coordinate,
			String relatedEntity, String property, boolean writeToFile) {
		List<String> properties = new ArrayList<String>();
		properties.add(property);
		writeLine(writer, entity, coordinate, relatedEntity, properties, writeToFile);
	}

	private void writeLine(PrintWriter writer, String entity, Coordinate coordinate, String relatedEntity,
			List<String> properties, boolean writeToFile) {
